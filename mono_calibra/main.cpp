...
void outputCameraParam(void)
{
    ...
    if (fs.isOpened())
    {
        ...
        fs<<"Camera_fx"<<R_cameraMatrix.at<double>(0,0);//.at()先行后列 (00)
        fs<<"Camera_fy"<<R_cameraMatrix.at<double>(1,1);//11
        fs<<"Camera_cx"<<R_cameraMatrix.at<double>(0,2);//02
        fs<<"Camera_cy"<<R_cameraMatrix.at<double>(1,2);//12

        fs<<"Camera_k1"<<R_distCoeffs.at<double>(0,0);//00
        fs<<"Camera_k2"<<R_distCoeffs.at<double>(0,1);//01
        fs<<"Camera_p1"<<R_distCoeffs.at<double>(0,2);//02
        fs<<"Camera_p2"<<R_distCoeffs.at<double>(0,3);//03
        fs<<"Camera_k3"<<R_distCoeffs.at<double>(0,4);//04


        fs<<"Camera_width"<<R_img.cols;
        fs<<"Camera_height"<<R_img.rows;
        fs<<"Camera_bf"<< fx*0.117;
        fs<<"Camera_fps"<<30;
        fs<<"Camera_RGB"<<1;
        fs<<"ThDepth"<<40.0;
        fs<<"DepthMapFactor"<<1000;
        // ORB特征设置
